...
class Image extends Base_Model
{
	...
	function delete($id)
	{
 	...
 	function delete_city_image_by_parent($parent_id)
	{
		$this->db->where('parent_id', $parent_id);
	...
	function get_item_images_by_city($city_id)
	{
		$this->db->from($this->table_name);
	...
	function get_cover_image($city_id,$type)
	{
		$query = $this->db->get_where($this->table_name,array('parent_id'=>$city_id, 'type'=>$type));
